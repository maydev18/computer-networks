...
class hamming{
    public:
        string data;   //it is the raw data recieved
        int n , r = 0; // n is the length of raw data and r is the number of redundant bits 
        char * msg; // it will store the all bits (data + redundant)
        hamming(){

            //taking input from user and initializing all data members
            cout << "enter the data : ";
            cin >> data;
        ...
        void setRedundantBits(){
            ...
            for(int i = 1 ; i  <= n+r ; i*=2){
                ...
                for(int j = i+1 ; j<=n+r ; j++){
                    ...
                    if(j & (1 << bit)){
                        if(msg[j] == '1') count++; // counting the number of ones in corrersponding data bits
                    }
                }
            ...
            //showing up the messsage to be sent(data + redundant)
            showmsg();
        }
        void receiver(){
            ...
            // if the ans had any occurence of 1 then there is some fault 
            if(ans.find('1') != string::npos){
                //reversing the ans
                reverse(ans.begin() , ans.end());
                int power = 1;
                int wrongbit = 0;
                //evaluationg the binary expression of ans variable
            ...
            // if the ans dont have any occurence of 1 then it is correct
            else{
                cout << "correct data packet recieved " << endl;
            }
        }
};
int main(){
    hamming h;
    // manipulating any ith data bit to check if receiver is detecting a error in that bit. If you elimate the following line then correct code will be sent to receiver following that no error is received
...
//this code is contirbuted by Mayank Sharma.